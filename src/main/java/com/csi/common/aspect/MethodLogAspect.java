...
import com.csi.common.baseenum.LogTypeEnum;
import com.csi.common.annotation.MethodLog;
import com.csi.config.StatsdAutoConfiguration.StatsdUtil;
...
public class MethodLogAspect{
	...
	@Pointcut("@annotation(com.csi.common.annotation.MethodLog)")
	...
	public Object around(ProceedingJoinPoint point) throws Throwable {
		...
		try {
			...
			long time = System.currentTimeMillis() - beginTime;
			//记录执行后返回日志
			resultMethodLog(point, result.toString(),time,number);
	...
	private void resultMethodLog(ProceedingJoinPoint joinPoint,Object result,long time,int number){
		...
		log.info("请求唯一标识->{}-{} 返回结果->{} 方法执行时间->{}",request.getAttribute(LogTypeEnum.WHICH_MARK_KEY.getValue()),number,result.toString(), time);
	}
