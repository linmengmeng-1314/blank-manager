$(function () {
	...
	layui.use('table', function() {
		...
		table.render({
			...
			url: baseURL + 'test/querytestList', //?page=1&limit=10（该参数可通过 request 自定义）
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.test.id? "test/edittest" : "test/savetest";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function() {
				$.ajax({
					type: "POST",
					url: baseURL + "test/deletetestList",
					contentType: "application/json",
		...
		getInfo: function(rowData){
			...
			$.ajax({
				type: "POST",
			    url: baseURL + "test/querytestList",
